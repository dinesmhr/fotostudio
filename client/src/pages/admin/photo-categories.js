...
const AdminPhotoCategories = (props) => {
	const [ productCats, setproductCats ] = useState([])
	const { userLoggedIn } = props

	useEffect(() => {
		let _this = this
        const url = 'fotostudio/api/photo-categories'
        axios.get( url )
								...
								productCats.map( ( productCat, index )  => {
									return (	 
										<tr key={ index }>
											...
											<td>
												<button><a href={ `/fstudio-photo-categories/:${productCat.ID}` }>{`Edit`}</a></button>
											</td>
										</tr>
									)
								})
...
export default AdminPhotoCategories;